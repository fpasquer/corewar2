...
int  						ft_indirect_param(t_vm *vm, t_player *plr, int move)
{
	...
	if (plr->instruction != LLD && plr->instruction != LLDI && plr->instruction != LFORK)
		i = get_new_index_with_mod(move, plr->i_grid, vm);
	else
		// i = ft_check_size_max2(move % 4096, plr->i_grid);
		i = get_new_index(move, plr->i_grid, vm);
	if (plr->instruction == STI || plr->instruction == LD || plr->instruction == LDI || plr->instruction == LLDI)
		i = ft_param_4_octets(vm, plr, 4, i);
	else
		i = ft_param_4_octets(vm, plr, 2, i);
	// i = ft_param_4_octets(vm, plr, plr->instruction != STI ? 4 : 2, i);
	return (i);
}
...
void 						ft_parse_info(t_vm *vm, t_player *plr)
{
	...
	{
		tmp->nb_f_param = tmp->t_f_param == T_IND ? ft_indirect_param(vm, plr, tmp->nb_f_param) : tmp->nb_f_param;
		tmp->nb_s_param = tmp->t_s_param == T_IND ? ft_indirect_param(vm, plr, tmp->nb_s_param) : tmp->nb_s_param;
		tmp->nb_t_param = tmp->t_t_param == T_IND ? ft_indirect_param(vm, plr, tmp->nb_t_param) : tmp->nb_t_param;
	}
}
...
int 						ft_ldi(t_vm *vm, t_player *plr)
{
	...
	plr->carry = (plr->instruction == LLDI && plr->reg[plr->info.reg_t]) ? 0 : 1;
	plr->do_instruction = 0;
	ft_bzero(&plr->info, sizeof(t_info));
	return (0);
}
...
// 	if (vm->array[plr->i_grid].code_hexa < 1 || vm->array[plr->i_grid].code_hexa > 15)
// 	{
// 		ft_nothing(vm, plr);
// 		plr->instruction = 0;
// 		return (0);
...
// 		{
// 			plr->instruction = vm->array[plr->i_grid].code_hexa;
